...
export default function DashboardView({
  ...
  if (loading) {
    return (
      <div className="py-20 flex items-center justify-center">
        <div className="flex items-center gap-3 px-4 py-2 text-gray-600 font-medium">
      ...
      <div className="py-16 text-center">
        <div className="max-w-md mx-auto bg-white border border-gray-200 p-8 rounded-2xl shadow-sm space-y-3">
          ...
          <p className="text-gray-600 font-medium">
            No pending dispatch data available.
          </p>
          {onRetry && (
            <button
              onClick={onRetry}
  ...
  const { peakDay, overdueOrdersCount, overdueQtyFormatted } =
    calculateDashboardMetrics(pendingDates);
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 p-5 rounded-xl shadow-xs hover:shadow-md transition-shadow duration-200 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-gray-800 capitalize">
              Total Pending Dispatch Quantity
            </span>
        ...
        <div className="bg-white border border-gray-200 p-5 rounded-xl shadow-xs hover:shadow-md transition-shadow duration-200 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-gray-800 capitalize">
              Date With Most Dispatch Orders
            </span>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-white flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 font-medium">
            {pendingDates.length}{" "}
            {pendingDates.length === 1 ? "dispatch date" : "dispatch dates"}
          </span>
        </div>

        {pendingDates.length === 0 ? (
          <p className="text-gray-500 italic p-6">
            No pending dispatches recorded.
          </p>
        ) : (
              ...
              return (
                <div key={day.dispatchDate} className="group flex flex-col">
                  <button
                    type="button"
                    onClick={() =>
                      setExpandedDay(isExpanded ? null : day.dispatchDate)
                    }
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <span
                        className={`font-medium text-sm ${isHighQty ? "text-red-800" : "text-gray-700"}`}
                      >
                        {day.dispatchDate}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-4 sm:gap-8 lg:gap-12">
                      ...
                      <span
                        className={`font-semibold text-right ${isHighQty ? "text-red-950" : "text-gray-700"}`}
                      >
                        {day.totalQty} MT
                      </span>
                    </div>
